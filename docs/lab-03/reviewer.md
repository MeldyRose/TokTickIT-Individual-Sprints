...
| 42 | feature/14-authentication | - Verified backend authentication endpoints (/api/auth/login, /api/auth/logout, /api/auth/me, /api/auth/change-password). - Session cookie handling correctly sets toktickit_session with HttpOnly flags. - mustChangePassword flag and bcrypt password hashing (salt 10) work as expected. - Inactive users and invalid credentials return safe 401 response without leaking user existence. Approved! |
...
| 44 | feature/16-requester-regression | - Temporary Lab 2 requester selector removed completely; user identity derived strictly from session context (AC-03, BR-22). - Requesters can post Public Comments and click "Problem Appears Resolved" to request status review. - Requesters attempting to access Internal Notes are rejected with 403 Forbidden. Tested and approved! |
...
| 47 | feature/19-comment-role-restricted | - Public Comments and Internal Notes sections rendered with distinct styling (green accent vs amber #FFFDE7 with lock icon 🔒 Internal Notes). - Internal Notes API (GET/POST /api/tickets/:id/notes) enforces strict server-side RBAC, blocking Requesters with 403. - All unit & API security probing tests pass. Approved for merge! |
...
| 49 | feature/21-e2e-inspection-docs | - Verified Playwright E2E test suites (authentication.spec.ts, staff-ticket-flow.spec.ts, user-administration.spec.ts) passing 100%. - Responsive component test (Responsive.test.tsx) verifies viewport layout shift (<768px). - Documentation matrix finalized and peer review recorded. Approved for release to main! |
